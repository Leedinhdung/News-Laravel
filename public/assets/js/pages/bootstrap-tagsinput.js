(function ($) {
    ...
    TagsInput.prototype = {
        ...
        add: function (item, dontPushVal, options) {
            ...
            var itemValue = self.options.itemValue(item), itemText = self.options.itemText(item),
                tagClass = self.options.tagClass(item), itemTitle = self.options.itemTitle(item);
        ...
        refresh: function () {
            ...
            $('.tag', self.$container).each(function () {
                var $tag = $(this), item = $tag.data('item'), itemValue = self.options.itemValue(item),
                    itemText = self.options.itemText(item), tagClass = self.options.tagClass(item);
        ...
        build: function (options) {
            ...
            self.$container.on('keydown', 'input', $.proxy(function (event) {
                ...
                var textLength = $input.val().length, wordSpace = Math.ceil(textLength / 5),
